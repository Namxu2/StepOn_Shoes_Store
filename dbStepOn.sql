...
CREATE TABLE `invoices` (
  ...
  `user_id` bigint DEFAULT NULL, -- Match the bigint type with users table's `id`
  ...
  `status` tinyint(1) DEFAULT NULL,
  PRIMARY KEY (`id`),
  FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) -- Correct table and column reference
);

CREATE TABLE `invoice_details` (
  `id` int AUTO_INCREMENT,
  `product_id` varchar(255) DEFAULT NULL, -- Khóa ngoại tham chiếu đến `product.id`
  `invoice_id` int DEFAULT NULL,          -- Khóa ngoại tham chiếu đến `invoices.id`
  `quantity` int DEFAULT NULL,
  `price` float DEFAULT NULL,
  `created_at` datetime DEFAULT NULL,
  `status` tinyint(1) DEFAULT NULL,
  PRIMARY KEY (`id`),
  FOREIGN KEY (`product_id`) REFERENCES `product`(`id`), -- Liên kết với bảng `product`
  FOREIGN KEY (`invoice_id`) REFERENCES `invoices`(`id`) -- Liên kết với bảng `invoices`
);
